...
C       Flip CM_RAP (to get rapidity right)
      ...
      IF (PASSCUTS(P1)) THEN
        IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! u s > c d chi chi~
        IF(IPROC.EQ.2) DSIGPROC=DSIG2(P1,WGT,IMODE)  ! u s~ > c d~ chi chi~
        IF(IPROC.EQ.3) DSIGPROC=DSIG3(P1,WGT,IMODE)  ! u~ s~ > c~ d~ chi chi~
      ENDIF
      RETURN
      END
